...
class F110Env(gym.Env):
    ...
    def __init__(self, **kwargs):
        # -------------------- MAP LOADING --------------------
        try:
            # 1. Retrieve the full map path. 
            # This line will raise KeyError if the 'map' argument is missing.
            map_full_path = str(kwargs['map']) # Example: '/absolute/path/to/vegas'
            # Add yaml to it
            if not map_full_path.endswith('.yaml'):
                map_full_path += '.yaml'


            # 2. Check if the file exists at the provided path
            if not os.path.exists(map_full_path):
                raise FileNotFoundError(f"Map file not found at: {map_full_path}")
            
            # 3. If valid, assign the path and extract a name.
            self.map_path = map_full_path
            
            # Extract map name from the path (e.g., 'vegas' from '.../vegas.yaml')
            # This helps with internal logging and rendering setup.
            base_name_with_ext = os.path.basename(self.map_path)
            self.map_name = os.path.splitext(base_name_with_ext)[0]
            
            # 4. Map image extension
            self.map_ext = kwargs.get('map_ext', '.png')
            
        except KeyError:
            # HALT: Compel user to provide the map argument
            raise KeyError("The required 'map' keyword argument is missing. Please provide the absolute YAML file path.")
        
        except FileNotFoundError as e:
            # HALT: Map file was not found at the provided path
            raise FileNotFoundError(f"Initialization Failed: {e}")
            
        except Exception as e:
            # Catch any other unexpected error during path handling and halt.
            raise RuntimeError(f"Map setup failed due to an unexpected error: {e}")
