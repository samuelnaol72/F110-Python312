...
# MODIFIED FILE: gym_bridge.py
# DESCRIPTION OF MODIFICATIONS:
# - Upgraded to Gymnasium API (import gymnasium as gym, gym.make, env.step, env.reset)
...
class GymBridge(Node):
    # =========================================================================
    # CORE NODE INITIALIZATION COMMENTARY:
    # 
    # The __init__ method is responsible for setting up the ROS 2 node, including:
    # 
    # 1. Parameter Declaration: Declaring all configuration parameters 
    #    (e.g., topics, namespaces, map info, initial poses) that are read from 
    #    the sim.yaml file. (self.declare_parameter)
    # 
    # 2. Initialization: Setting up the F1TENTH gym environment (gym.make) 
    #    and initial pose variables (self.ego_pose, etc.).
    # 
    # 3. Timers: Creating timers for the fixed-rate simulation step (drive_timer)
    #    and the ROS topic publishing loop (timer).
    # 
    # 4. Publishers: Creating publishers to send out data (LaserScan, Odometry).
    #    (self.create_publisher)
    # 
    # 5. Subscribers: Creating subscribers to receive commands (AckermannDriveStamped,
    #    /initialpose, /cmd_vel). This links incoming data to callback methods.
    #    (self.create_subscription)
    # 
    # 6. Transform Broadcaster: Initializing the TF broadcaster to publish 
    #    coordinate frame transformations (e.g., base_link to laser).
    # 
    # =========================================================================
    ...
    def __init__(self):
        ...
        self.declare_parameter('max_steer_angle', 0.35) # in radians
        ...
        # --- INITIAL RESET LOGIC ---
        initial_ego_pose = np.array([[sx, sy, stheta]])
        ...
        if num_agents == 2:
            ...
            initial_opp_pose = np.array([[sx1, sy1, stheta1]])
            full_initial_poses = np.concatenate((initial_ego_pose, initial_opp_pose))
            
            # UPGRADED: Reset takes pose array via options, returns (obs, info)
            self.obs, _ = self.env.reset(options={'poses': full_initial_poses})
            self.done = False 
            ...
            self.has_opp = False
            # UPGRADED: Reset takes pose array via options, returns (obs, info)
            self.obs, _ = self.env.reset(options={'poses': initial_ego_pose})
            self.done = False
            self.truncated = False
            self.ego_scan = list(self.obs['scans'][0])
        ...
        if self.get_parameter('kb_teleop').value:
            self.get_logger().info("kb_teleop parameter is TRUE - attempting to subscribe to /cmd_vel")
            try:
                ...
                self.get_logger().info("✓ Keyboard teleoperation ENABLED - subscribed to /cmd_vel")
            except Exception as e:
                self.get_logger().error(f"Failed to subscribe to /cmd_vel: {e}")
        else:
            self.get_logger().info("✗ Keyboard teleoperation DISABLED (kb_teleop=False)")


    # =========================================================================
    # EGO AGENT (Car 1) CALLBACKS
    # =========================================================================

    def drive_callback(self, drive_msg): # First agent drive callback (AckermannDriveStamped)
        self.ego_requested_speed = drive_msg.drive.speed
        self.ego_steer = drive_msg.drive.steering_angle
        self.ego_drive_published = True

    def ego_reset_callback(self, pose_msg):  # First agent reset callback (/initialpose)
        ...
        new_ego_pose = np.array([[rx, ry, rtheta]])
        reset_options = {'poses': new_ego_pose}

        if self.has_opp:
            opp_pose_arr = np.array([[self.obs['poses_x'][1], self.obs['poses_y'][1], self.obs['poses_theta'][1]]])
            full_reset_poses = np.concatenate((new_ego_pose, opp_pose_arr))
            reset_options = {'poses': full_reset_poses}

        # UPGRADED: Reset returns (obs, info)
        self.obs, _ = self.env.reset(options=reset_options)
        self.done = False # Reset done flag
        self.truncated = False
        self._update_sim_state()

    def teleop_callback(self, twist_msg): # Keyboard Teleop callback (/cmd_vel)
        if not self.ego_drive_published:
            self.ego_drive_published = True
            self.get_logger().info("Teleop: ego_drive_published set to True")

        self.ego_requested_speed = float(twist_msg.linear.x)
        self.ego_steer = float(twist_msg.angular.z)
        
        # Clamp steering angle using declared parameter
        self.ego_steer = max(-self.max_steer_angle, min(self.max_steer_angle, self.ego_steer))
        
        self.get_logger().info(f"Teleop received: speed={self.ego_requested_speed:.2f}, steer={self.ego_steer:.2f}")

    # =========================================================================
    # OPPONENT AGENT (Car 2) CALLBACKS
    # =========================================================================

    def opp_drive_callback(self, drive_msg):  # Second agent drive callback (AckermannDriveStamped)
        self.opp_requested_speed = drive_msg.drive.speed
        self.opp_steer = drive_msg.drive.steering_angle
        self.opp_drive_published = True

    def opp_reset_callback(self, pose_msg): # Second agent reset callback (/goal_pose)
        if self.has_opp:
            ...
            new_opp_pose = np.array([[rx, ry, rtheta]])
            ego_pose_arr = np.array([list(self.ego_pose)])
            full_reset_poses = np.concatenate((ego_pose_arr, new_opp_pose))
            
            # UPGRADED: Reset returns (obs, info)
            self.obs, _ = self.env.reset(options={'poses': full_reset_poses})
            self.done = False # Reset done flag
            self.truncated = False
            self._update_sim_state()

    # =========================================================================
    # COMMON/UTILITY FUNCTIONS (Timers, Publishers, State Update)
    # =========================================================================


    # This function is called at a fixed rate to step the simulation
    # based on the latest drive commands received and stored in the class variables.
    def drive_timer_callback(self):
        # UPGRADED: Step now returns 5 values (obs, reward, terminated, truncated, info)
        
        terminated = False  # Default value if no step is executed
        
        self.get_logger().debug(f"drive_timer: ego_drive_published={self.ego_drive_published}, has_opp={self.has_opp}")
        
        if self.ego_drive_published and not self.has_opp:
            # Note: We assign 'terminated' to self.done for ROS compatibility
            self.get_logger().info(f"Stepping with: steer={self.ego_steer:.2f}, speed={self.ego_requested_speed:.2f}")
            self.obs, _, terminated, self.truncated, _ = self.env.step(np.array([[self.ego_steer, self.ego_requested_speed]]))
        elif self.ego_drive_published and self.has_opp and self.opp_drive_published:
            self.obs, _, terminated, self.truncated, _ = self.env.step(np.array([[self.ego_steer, self.ego_requested_speed], [self.opp_steer, self.opp_requested_speed]]))
        else:
            self.get_logger().debug(f"Step skipped: ego_drive={self.ego_drive_published}, has_opp={self.has_opp}")
        
        self.done = terminated # Use terminated flag for collision/finish

        # If the simulator indicates the episode is over (collision or lap finish)
        if self.done or self.truncated:
            self.get_logger().info("Episode finished. Resetting environment.")
            
            reset_pose = np.array([[self.ego_pose[0], self.ego_pose[1], self.ego_pose[2]]])
            
            if self.has_opp:
                opp_pose = np.array([[self.opp_pose[0], self.opp_pose[1], self.opp_pose[2]]])
                full_reset_poses = np.concatenate((reset_pose, opp_pose))
                # UPGRADED: Reset returns (obs, info)
                self.obs, _ = self.env.reset(options={'poses': full_reset_poses})
            else:
                # UPGRADED: Reset returns (obs, info)
                self.obs, _ = self.env.reset(options={'poses': reset_pose})
    ...
    # This timer function is called at a fixed rate to publish ROS topics
